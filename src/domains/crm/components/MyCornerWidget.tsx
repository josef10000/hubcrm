...
import { PHILOSOPHICAL_QUOTES } from '../../nexus/constants/philosophicalQuotes';
...
export default function MyCornerWidget() {
  ...
  // Lógica determinística para a frase filosófica do dia (Diferente por pessoa, fixa por dia)
  React.useEffect(() => {
    if (!user) return;
    ...
    // Algoritmo Determinístico: Usa a data e o UID do usuário para escolher uma frase da lista filosófica
    ...
    const targetQuote = PHILOSOPHICAL_QUOTES[index];
    setDailyQuote(targetQuote);
    
    // Salva a frase no cache para evitar recálculos no mesmo dia
    localStorage.setItem(storageKey, JSON.stringify({ date: today, quote: targetQuote }));
  }, [user]);
      ...
      {!isEditing ? (
        ...
        <div className="space-y-4">
          <div className="p-4 bg-primary-500/5 border border-primary-500/10 rounded-xl">
            <p className="text-[10px] font-black text-primary-500 uppercase tracking-widest mb-1">Citação Filosófica do Dia</p>
            <p className="text-xs text-gray-500 dark:text-gray-400 italic">Cada colega recebe uma dose única e personalizada de filosofia ou psicologia a cada 24 horas.</p>
          </div>
          <div>
