...
export default function ProductPortfolioPanel({ offers, canManage, onSave }: Props) {
  ...
  const groups = useMemo(() => {
    const result: Record<Segment, Offer[]> = { B2B: [], B2C: [], OTHER: [] };
      ...
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-5 mb-6">
        <div>
          <div className="flex items-center gap-2 text-primary-400 mb-1">
            <BarChart3 size={18} />
            <span className="text-[10px] font-black uppercase tracking-[0.18em]">Visão estratégica</span>
          </div>
        ...
        <div className="flex items-center gap-2 text-[11px] font-semibold text-gray-500 dark:text-gray-400">
          <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-emerald-400" /> Produção</span>
          <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-amber-400" /> Desenvolvimento</span>
          <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-sky-400" /> Validação</span>
        </div>
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-[1.15fr_1fr] gap-5">
        <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
          <div className="flex items-center gap-2 mb-4 text-xs font-black uppercase tracking-wider text-gray-500 dark:text-gray-400">
            <Target size={14} /> Mapa da empresa
          </div>
          <div className="flex justify-center mb-5">
            <div className="px-5 py-2.5 rounded-2xl border border-primary-500/30 bg-primary-500/10 text-sm font-black text-primary-400">HUB SYMPLES</div>
          </div>
            ...
            {(['B2B', 'B2C'] as Segment[]).map(segment => (
              <div key={segment} className="rounded-2xl border border-white/10 bg-black/20 p-4">
                <div className="flex items-center gap-2 mb-3">
                  <CircleDot size={14} className={segment === 'B2B' ? 'text-indigo-400' : 'text-pink-400'} />
                  <span className="font-black text-sm text-gray-900 dark:text-white">{segment}</span>
                  <span className="ml-auto text-[10px] font-bold text-gray-500">{groups[segment].length} produto(s)</span>
                </div>
                <div className="space-y-2">
                  {groups[segment].map(offer => {
                    const status = (offer.portfolioStatus || 'PRODUCTION') as ProductStatus;
                    return (
                      <button key={offer.id} onClick={() => { setSelectedId(offer.id); setEditing(false); }} className={`w-full flex items-center gap-2 rounded-xl px-3 py-2.5 text-left border transition-all ${selected?.id === offer.id ? 'border-primary-500/40 bg-primary-500/10' : 'border-transparent bg-white/5 hover:bg-white/10'}`}>
                        <span className={`h-2 w-2 rounded-full ${statusMeta[status].dot}`} />
                        <span className="text-xs font-bold text-gray-800 dark:text-gray-200 truncate">{offer.name}</span>
          ...
          {!!groups.OTHER.length && (
            <div className="mt-4 text-[11px] text-gray-500">{groups.OTHER.length} produto(s) ainda sem segmento B2B/B2C.</div>
          )}
        </div>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`h-2.5 w-2.5 rounded-full ${statusMeta[selectedStatus].dot}`} />
                      <span className="text-[10px] font-black uppercase tracking-wider text-gray-500">{statusMeta[selectedStatus].label}</span>
                    </div>
                    <h4 className="text-lg font-black text-gray-900 dark:text-white">{selected.name}</h4>
                    <span className="inline-flex mt-2 px-2 py-1 rounded-lg bg-white/5 border border-white/10 text-[10px] font-bold text-gray-500 dark:text-gray-400">{segmentLabel(selected.portfolioSegment as Segment)}</span>
                  </div>
                  {canManage && <button onClick={() => setEditing(true)} className="p-2 rounded-xl border border-white/10 text-gray-500 hover:text-primary-400 hover:bg-white/5"><Pencil size={15} /></button>}
                </div>

                <div className="mt-6">
                  <div className="flex justify-between text-[11px] font-bold mb-2"><span className="text-gray-500">Progresso estratégico</span><span className="text-gray-800 dark:text-gray-200">{selected.portfolioProgress ?? 0}%</span></div>
                  <div className="h-2 rounded-full bg-black/20 overflow-hidden"><div className="h-full rounded-full bg-primary-500 transition-all" style={{ width: `${selected.portfolioProgress ?? 0}%` }} /></div>
                </div>

                <div className="mt-5 rounded-xl border border-white/10 bg-black/10 p-4">
                  <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-wider text-gray-500 mb-2"><Target size={13} /> Objetivo atual</div>
                  <p className="text-sm text-gray-700 dark:text-gray-200 leading-relaxed">{selected.strategicObjective || 'Defina o objetivo estratégico deste produto.'}</p>
                </div>
                <div className="mt-3 rounded-xl border border-white/10 bg-black/10 p-4">
                  <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-wider text-gray-500 mb-2"><Flag size={13} /> Próximo marco</div>
                  <p className="text-sm font-semibold text-gray-700 dark:text-gray-200">{selected.nextMilestone || 'Nenhum marco definido.'}</p>
                </div>
              </>
            ) : (
              <form onSubmit={save}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <label className="text-[11px] font-bold text-gray-500">Segmento<select name="segment" defaultValue={selected.portfolioSegment || 'OTHER'} className="mt-1 w-full rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-xs text-gray-900 dark:text-white"><option value="B2B">B2B</option><option value="B2C">B2C</option><option value="OTHER">Outro</option></select></label>
                  <label className="text-[11px] font-bold text-gray-500">Status<select name="status" defaultValue={selected.portfolioStatus || 'PRODUCTION'} className="mt-1 w-full rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-xs text-gray-900 dark:text-white"><option value="IDEA">Ideia</option><option value="VALIDATION">Validação</option><option value="DEVELOPMENT">Desenvolvimento</option><option value="PRODUCTION">Produção</option><option value="PAUSED">Pausado</option></select></label>
                </div>
