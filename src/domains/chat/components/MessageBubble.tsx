...
const AudioPlayer = ({ 
  ...
  return (
    <div className={`p-4 rounded-[2rem] ${bubbleBg} max-w-sm w-full flex flex-col gap-3 shadow-lg animate-in fade-in duration-300 relative z-30`}>
      ...
      {transcription && (
        <div className="mt-2 border-t border-white/10 dark:border-white/5 pt-2">
          <button
            onClick={() => setShowTranscription(!showTranscription)}
            className={`w-full flex items-center justify-between text-[11px] font-black uppercase tracking-wider py-1 px-2 rounded-lg transition-all ${
              isMine
          ...
          >
            ...
            <svg
              ...
              strokeWidth="2.5"
              stroke="currentColor"
              className={`w-3.5 h-3.5 transition-transform duration-200 ${showTranscription ? 'rotate-180' : ''}`}
            >
          ...
          {showTranscription && (
            <div className={`mt-2 p-3 rounded-xl border text-[11px] leading-relaxed relative group/trans ${
              isMine
                ? 'bg-white/5 border-white/10 text-white/90 backdrop-blur-md'
                : 'bg-black/5 dark:bg-white/5 border-black/5 dark:border-white/10 text-gray-700 dark:text-gray-200 backdrop-blur-md'
            }`}>
              <p className="pr-6 whitespace-pre-wrap select-text font-medium text-left">{transcription}</p>
              ...
              <button
                ...
                className={`absolute top-2 right-2 p-1.5 rounded-lg border opacity-0 group-hover/trans:opacity-100 focus:opacity-100 transition-all ${
                  isMine
                    ? 'bg-zinc-800 border-white/10 text-white hover:bg-zinc-700'
                    : 'bg-white dark:bg-zinc-800 border-gray-200 dark:border-white/10 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-zinc-700'
                }`}
