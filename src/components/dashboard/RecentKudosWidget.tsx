...
import { Heart, Star, MessageSquare, Globe, ArrowRight, User as UserIcon } from 'lucide-react';
...
export function RecentKudosWidget() {
  ...
  useEffect(() => {
    ...
    // Busca apenas feedbacks públicos, focando em Elogios e PDIs
    const q = query(
  ...
  const getTypeColor = (type: FeedbackNote['type']) => {
    switch (type) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4 px-2">
        ...
        <span className="text-[10px] font-bold text-gray-500 bg-gray-100 dark:bg-white/5 px-3 py-1 rounded-full border border-gray-200 dark:border-white/10 uppercase tracking-widest">
          Feed em Tempo Real
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <AnimatePresence mode='popLayout'>
          {recentKudos.map((kudo, index) => {
            ...
            const target = teamProfiles.find(p => p.uid === kudo.userId);
            const Icon = getTypeIcon(kudo.type);
            const colorClass = getTypeColor(kudo.type);
            ...
            return (
              <motion.div
                ...
                className="bg-white/40 dark:bg-black/20 backdrop-blur-xl border border-gray-200 dark:border-white/10 p-5 rounded-[2rem] shadow-lg hover:shadow-primary-500/5 transition-all group overflow-hidden relative"
              >
                {/* Background Spark */}
                <div className="absolute -right-4 -top-4 w-24 h-24 bg-primary-500/5 blur-3xl rounded-full group-hover:bg-primary-500/10 transition-all"></div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] text-gray-500">reconheceu</span>
                    <span className="text-[10px] font-bold text-primary-500">{target?.displayName?.split(' ')[0]}</span>
                  </div>
