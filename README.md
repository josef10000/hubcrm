...
### 🎵 Hub Focus Station v2.0 (Streaming Híbrido: Spotify SDK/Embed, YouTube & Lofi Focus Beats)
...
- **Três Abas Independentes:** Seletor limpo e fluido com três áreas distintas:
  - `🧠 Focus Vibes`: Sons instrumentais de Lofi relaxantes tocados via streaming estável de áudio HTML5 local.
  - `🎵 Spotify`: Player com suporte híbrido inteligente de alta fidelidade.
  - `📺 YouTube`: Integração de reprodutor de vídeo para faixas ou playlists externas do YouTube.
- **Modo Híbrido Inteligente do Spotify:** Permite alternar de forma transparente entre dois modos de funcionamento na aba do Spotify:
  - **Modo Embed (Gratuito):** Renderiza o iframe clássico do Spotify de forma elegante e integrada para qualquer tipo de conta (Free ou Premium), sem necessidade de login.
  - **Modo SDK Premium (Controle Nativo):** Conecta-se ao **Spotify Web Playback SDK** oficial. Requer conta Premium e login inicial. Oferece controle total nativo direto no CRM (Play/Pause, barra de progresso da faixa, slider de volume real de 0% a 100%, capa e nome da faixa ativa).
- **Fluxo OAuth Implicit Grant Integrado:** O player realiza o login oficial do Spotify através do Implicit Grant redirecionando para a URL de produção homologada `https://hubcrm.hubsymples.com.br/`. Ao retornar, o token de acesso é capturado no hash, persistido na store e a URL é imediatamente limpa via `window.history.replaceState` mantendo um visual impecável.
- **Playlist Corporativa da Empresa:** Exibe nativamente a playlist oficial corporativa `Hub SiYmples` para reprodução instantânea com um único clique.
...
- **Filtragem Dinâmica & Contextual de Playlists:** A tela de gerenciamento de playlists (`RealRadiosTab.tsx`) agora filtra automaticamente a lista de favoritos e playlists personalizadas do usuário de acordo com a plataforma correspondente selecionada (exibindo apenas Spotify na aba do Spotify e YouTube na aba do YouTube), otimizando também o formulário com placeholders e validações contextuais separadas para cada tipo.
