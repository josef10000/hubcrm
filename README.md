...
- **Cloudflare R2:** Provedor de armazenamento em nuvem S3-compatible utilizado para guardar arquivos pesados de PDFs e documentos da biblioteca, além de servir como repositório central de mídias de chat (áudios, imagens, PDFs e planilhas).
  ...
  >   - 🎙️ **Mensagens de Voz Premium & Player Waveform:** Gravação nativa de áudio (usando `MediaRecorder` nativo) revestida em uma interface de **Glassmorphism Premium** com desfoque de fundo e uma incrível **Waveform de onda sonora em tempo real** desenhada com gradientes fluidos e dinâmicos que acompanham perfeitamente o tema ativo selecionado no CRM (seja azul, verde, laranja, cyberpunk, branco-elite, etc.). A gravação conta com um sistema de cancelamento robusto e à prova de falhas com descarte instantâneo de streaming. No chat, os áudios ganham um player estético com waveform interativa e dinâmica para navegação, controle de velocidade de reprodução aprimorado (1x, 1.5x, 2x) e micro-animações táteis. Os arquivos WebM gravados agora salvam sua duração em segundos codificada no nome do arquivo, sendo decodificados instantaneamente por meio de Regex no frontend para exibir a duração total correta imediatamente ao carregar, evitando o erro de `Infinity` ou `NaN` no player.
  ...
  >     - 📥 **Seleção Múltipla & Ações em Lote:** Seleção de múltiplas mensagens com suporte a ações em lote direto na barra inferior de controle (Copiar textos, Excluir em massa e Encaminhar mensagens selecionadas para múltiplos chats simultaneamente).
  ...
  >     - 🎙️ **Transcrição de Áudio por IA:** Transcrição instantânea de mensagens de voz e áudios com um único clique. Integração de IA robusta processando o arquivo de áudio e exibindo a transcrição de forma elegante e colapsável sob o player waveform.
  ...
  >     - ⚡ **Seletor de Atalhos, Prioridades & Templates:** Barra de controle no editor permitindo a seleção visual de prioridade, templates rápidos (⚡) para agilizar conversas recorrentes, botão de checklist, além do atalho `ArrowUp` para editar rapidamente a última mensagem enviada.
