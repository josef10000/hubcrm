...
- **Cloudflare R2:** Provedor de armazenamento em nuvem S3-compatible utilizado para guardar arquivos pesados de PDFs e documentos da biblioteca, além de servir como repositório central de mídias de chat (áudios, imagens, PDFs e planilhas).
  ...
  >     - ⚡ **Seletor de Atalhos, Prioridades & Templates (Fallback Resiliente):** Barra de controle no editor permitindo a seleção visual de prioridade, templates rápidos (⚡) com fallback estático de alta qualidade para agilizar conversas recorrentes mesmo se o cache local estiver vazio, botão de checklist, além do atalho `ArrowUp` para editar rapidamente a última mensagem enviada.
