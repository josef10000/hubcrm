...
### 🎵 Hub Focus Station (Streaming Híbrido: Spotify, YouTube & Lofi Focus Beats)
...
- **Aba "Streaming" Unificada:** A aba de "Spotify" foi renomeada para "Streaming" no painel principal, centralizando de forma coesa ambas as plataformas em uma experiência premium.
- **Focus Spotify Station (Widget Ampliado de 352px):** O player do Spotify foi expandido verticalmente de 80px para exatamente **352px**. Isso permite que os usuários visualizem a playlist completa, a listagem de faixas do álbum e tenham acesso direto ao slider e controles de volume nativos do Spotify.
- **Integração Premium com YouTube (Vídeos e Playlists):** Suporte nativo completo a links do YouTube. Ao selecionar uma estação do YouTube, a aplicação faz a comunicação com o iframe via API de JS (`postMessage` com o parâmetro `enablejsapi=1`), permitindo:
  - **Controle Fino de Volume:** Controle e sincronização suave do volume (0-100%) integrado ao slider global do CRM.
  - **Comandos de Play/Pause:** Pausar e retomar a música do YouTube de forma síncrona com os botões gerais do Focus Station.
  - **Capas Dinâmicas de Vídeo:** Obtenção automática e instantânea das capas de vídeos e playlists do YouTube usando o resolvedor local e imagens estáticas oficiais (`img.youtube.com/vi/...`).
