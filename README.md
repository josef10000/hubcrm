...
*   **Autenticação Geral Obrigatória (Login no Início):** A rota principal do portal (`/portal/:orgId/:clientId`) exige autenticação obrigatória logo no carregamento. Usuários não autenticados ou não autorizados são impedidos de visualizar qualquer informação (incluindo a Dashboard) e são apresentados a um painel de bloqueio unificado Glassmorphism. A segurança foi endurecida para exigir login de todos os usuários (incluindo administradores corporativos em ambiente de teste), evitando bypass acidental por sessões administrativas ativas.
*   **Validação de E-mail em Tempo Real (Auto-Revogação):** O portal valida continuamente o e-mail do Firebase Auth contra o e-mail registrado no card do cliente no CRM. Se o administrador alterar o e-mail no painel administrativo, o sistema detecta a divergência instantaneamente, remove o perfil `/profiles/{uid}` obsoleto do Firestore e encerra a sessão do usuário, exigindo login com o novo e-mail cadastrado.
*   **Agenda de Atendimentos com CRUD Completo:** Aba com visualização linear e cronológica diária dos horários agendados por clientes. Permite aos atendentes gerenciar compromissos, contatar clientes via WhatsApp, criar novos atendimentos e agora **editar ou excluir** agendamentos existentes (com reflexão imediata nos relatórios financeiros).
*   **Gestão Contábil de Receitas Manuais e Despesas:** Painel financeiro reativo que calcula faturamento projetado, receitas efetivadas (pagas), despesas de operação (gastos) e o lucro líquido real do período filtrado. Além de gastos, permite **registrar receitas manuais** (como mensalidades, vendas de produtos e serviços adicionais pontuais/fixos) na mesma visualização.
...
*   **Sidebar de Luxo e Experiência Visual Premium:** Nova sidebar refinada integrada ao Portal do Cliente. Possui textura de ruído fotográfico (`grain overlay`), efeito Glassmorphism avançado, pílula de seleção deslizante e indicador lateral neon dinâmico através do Framer Motion (usando `layoutId` para transição sem interrupção). O rodapé exibe o avatar do cliente com glow reativo esmeralda (status Ativo) ou âmbar (outros status), acompanhado de uma badge de plano estilizada.
