...
- **Intelligent Iframe Kill Switch (Spotify):** Por rodar em sandbox cross-origin, o Iframe do Spotify ignora a maioria dos comandos de volume e pausa nativos por razões de segurança. Para sanar isso, implementamos uma técnica reativa no React baseada no ciclo de vida de chaves (`key`). Assim que uma chamada WebRTC é iniciada (`ringing` ou `connected`), o áudio é mutado ou o slider de volume é zerado, a `key` do iframe é alterada dinamicamente, forçando o navegador a destruir e recriar o iframe no mesmo milissegundo, silenciando o player de forma infalível e instantânea sem atrapalhar a operação comercial.
